...
def remove_singlets(temp_mask, mosaic_bw, filenames, current_upper_x, current_upper_y, tfws, fill_dict, x_disp, y_disp, completed_images, drawerr, prev_img, proc_imgs):
    repeat = 0
    temp_mask_ret = np.zeros(
        (temp_mask.shape[0], temp_mask.shape[1]), np.uint8)
    threes = np.where(temp_mask > 0)
    dimxmin = min(threes[0])-2
    dimxmax = max(threes[0])+2
    dimymin = min(threes[1])-2
    dimymax = max(threes[1])+2
    ...
    h, w = blobcolor.shape
    seed = (1, 1)
    maskall = np.zeros((h+2, w+2), np.uint8)
    floodflags = 4
    floodflags |= cv2.FLOODFILL_MASK_ONLY
    floodflags |= (255 << 8)
    num, im, maskall, rect = cv2.floodFill(
        blobcolor, maskall, seed, 255, 0, 0, floodflags)
    maskall = maskall[1:-1, 1:-1]
    maskall = cv2.bitwise_not(maskall)
    maskall[temp_mask_ret[max(dimxmin-1, 0):min(dimxmax+1, temp_mask_ret.shape[0]),
                          max(dimymin-1, 0):min(dimymax+1, temp_mask_ret.shape[1])] > 0] = 0
    for x in range(len(threes[0])):
        if(neighbors8_ex(temp_mask_ret, threes[0][x], threes[1][x]) > 0):
            temp_mask[threes[0][x]][threes[1][x]] = 254
    maskall[temp_mask[max(dimxmin-1, 0):min(dimxmax+1, temp_mask_ret.shape[0]),
                      max(dimymin-1, 0):min(dimymax+1, temp_mask_ret.shape[1])] == 254] = 0
    ret, labels = cv2.connectedComponents(maskall)
    cont_lab = 0
            ...
            if(lab != 0):
                ...
                for filcont in range(4):
                    if filled == 1:
                        break
                    if(filcont == 0):
                        dimy = x_disp+max(dimxmin-1, 0) + \
                            dims[0][np.argmin(dims[0])]
                        dimx = y_disp+max(dimymin-1, 0) + \
                            dims[1][np.argmin(dims[0])]
                    if(filcont == 1):
                        dimy = x_disp+max(dimxmin-1, 0) + \
                            dims[0][np.argmax(dims[0])]
                        dimx = y_disp+max(dimymin-1, 0) + \
                            dims[1][np.argmax(dims[0])]
                    if(filcont == 2):
                        dimy = x_disp+max(dimxmin-1, 0) + \
                            dims[0][np.argmin(dims[1])]
                        dimx = y_disp+max(dimymin-1, 0) + \
                            dims[1][np.argmin(dims[1])]
                    if(filcont == 3):
                        dimy = x_disp+max(dimxmin-1, 0) + \
                            dims[0][np.argmax(dims[1])]
                        dimx = y_disp+max(dimymin-1, 0) + \
                            dims[1][np.argmax(dims[1])]
                    for dircont in range(8):
                        ...
                        if(dircont == 7):
                            dimy2 = dimy+1
                            dimx2 = dimx-1
                        if(prev_img == False and mosaic_bw[dimy2][dimx2] == 0 and dircont in [1, 2, 4, 5]):
                            ...
                            continue
                        if(mosaic_bw[dimy2][dimx2] != 0 and mosaic_bw[dimy2][dimx2] != proc_imgs and (counts < 1 or (fill_dict[dimy][dimx] is None or fill_dict[dimy][dimx][mosaic_bw[dimy2][dimx2]-1] is None or counts not in fill_dict[dimy][dimx][mosaic_bw[dimy2][dimx2]-1])) and (counts < 1000 or check_bounds(mosaic_bw, dims, mosaic_bw[dimy2][dimx2], current_upper_x, current_upper_y, seen, tfws, y_disp+max(dimymin-1, 0), x_disp+max(dimxmin-1, 0)))):
                            mosaic_bw[int(x_disp):int(x_disp)+temp_mask_ret.shape[0], int(y_disp):int(y_disp)+temp_mask_ret.shape[1]][max(dimxmin-1, 0):min(
                                dimxmax+1, temp_mask_ret.shape[0]), max(dimymin-1, 0):min(dimymax+1, temp_mask_ret.shape[1])][dims] = mosaic_bw[dimy2][dimx2]
    ...
    for x in range(len(threes[0])):
        if(neighbors8(temp_mask, threes[0][x], threes[1][x]) == 1):
            ...
            if(drawerr != 2 and mosaic_bw[dimy][dimx] == proc_imgs):
                proc_imgs2 = max(Counter([mosaic_bw[dimy][dimx+1] if mosaic_bw[dimy][dimx+1] != proc_imgs else 0, mosaic_bw[dimy][dimx-1] if mosaic_bw[dimy][dimx-1] != proc_imgs else 0,
                                          mosaic_bw[dimy-1][dimx] if mosaic_bw[dimy-1][dimx] != proc_imgs else 0, mosaic_bw[dimy+1][dimx] if mosaic_bw[dimy+1][dimx] != proc_imgs else 0]).most_common(2))[0]
                if(proc_imgs2 != 0):
                    if(fill_dict[dimy][dimx] is None):
                        fill_dict[dimy][dimx] = [
                            None for xn in range(len(filenames))]
                        ...
                        if(fill_dict[dimy][dimx][proc_imgs2-1] is None):
                            ...
                            if 1 not in fill_dict[dimy][dimx][proc_imgs2-1]:
                                ...
                                proc_imgs2 = max(Counter([mosaic_bw[dimy][dimx+1] if mosaic_bw[dimy][dimx+1] != 0 and mosaic_bw[dimy][dimx+1] != proc_imgs2 and (fill_dict[dimy][dimx][mosaic_bw[dimy][dimx+1]-1] is None or 1 not in fill_dict[dimy][dimx][mosaic_bw[dimy][dimx+1]-1]) else 0, mosaic_bw[dimy][dimx-1] if mosaic_bw[dimy][dimx-1] != 0 and mosaic_bw[dimy][dimx-1] != proc_imgs2 and (fill_dict[dimy][dimx][mosaic_bw[dimy][dimx-1]-1] is None or 1 not in fill_dict[dimy][dimx][mosaic_bw[dimy][dimx-1]-1])
                                                          else 0, mosaic_bw[dimy-1][dimx] if mosaic_bw[dimy-1][dimx] != 0 and mosaic_bw[dimy-1][dimx] != proc_imgs2 and (fill_dict[dimy][dimx][mosaic_bw[dimy-1][dimx]-1] is None or 1 not in fill_dict[dimy][dimx][mosaic_bw[dimy-1][dimx]-1]) else 0, mosaic_bw[dimy+1][dimx] if mosaic_bw[dimy+1][dimx] != 0 and mosaic_bw[dimy+1][dimx] != proc_imgs2 and (fill_dict[dimy][dimx][mosaic_bw[dimy+1][dimx]-1] is None or 1 not in fill_dict[dimy][dimx][mosaic_bw[dimy+1][dimx]-1]) else 0]).most_common(2))[0]
                                if(proc_imgs2 == 0 and 0 not in [mosaic_bw[dimy][dimx+1], mosaic_bw[dimy][dimx-1], mosaic_bw[dimy-1][dimx], mosaic_bw[dimy+1][dimx]]):
                                    fill_dict[dimy][dimx][mosaic_bw[dimy][dimx+1]-1] = [] if fill_dict[dimy][dimx][mosaic_bw[dimy]
                                                                                                                   [dimx+1]-1] is None else [x for x in fill_dict[dimy][dimx][mosaic_bw[dimy][dimx+1]-1] if x != 1]
                                    fill_dict[dimy][dimx][mosaic_bw[dimy][dimx-1]-1] = [] if fill_dict[dimy][dimx][mosaic_bw[dimy]
                                                                                                                   [dimx-1]-1] is None else [x for x in fill_dict[dimy][dimx][mosaic_bw[dimy][dimx-1]-1] if x != 1]
                                    fill_dict[dimy][dimx][mosaic_bw[dimy-1][dimx]-1] = [] if fill_dict[dimy][dimx][mosaic_bw[dimy-1]
                                                                                                                   [dimx]-1] is None else [x for x in fill_dict[dimy][dimx][mosaic_bw[dimy-1][dimx]-1] if x != 1]
                                    fill_dict[dimy][dimx][mosaic_bw[dimy+1][dimx]-1] = [] if fill_dict[dimy][dimx][mosaic_bw[dimy+1]
                                                                                                                   [dimx]-1] is None else [x for x in fill_dict[dimy][dimx][mosaic_bw[dimy+1][dimx]-1] if x != 1]
                                    proc_imgs2 = max(Counter([mosaic_bw[dimy][dimx+1] if mosaic_bw[dimy][dimx+1] != 0 and mosaic_bw[dimy][dimx+1] != proc_imgs2 and (fill_dict[dimy][dimx][mosaic_bw[dimy][dimx+1]-1] is None or 1 not in fill_dict[dimy][dimx][mosaic_bw[dimy][dimx+1]-1]) else 0, mosaic_bw[dimy][dimx-1] if mosaic_bw[dimy][dimx-1] != 0 and mosaic_bw[dimy][dimx-1] != proc_imgs2 and (fill_dict[dimy][dimx][mosaic_bw[dimy][dimx-1]-1] is None or 1 not in fill_dict[dimy][dimx][mosaic_bw[dimy][dimx-1]-1]) else 0,
                                                              mosaic_bw[dimy-1][dimx] if mosaic_bw[dimy-1][dimx] != 0 and mosaic_bw[dimy-1][dimx] != proc_imgs2 and (fill_dict[dimy][dimx][mosaic_bw[dimy-1][dimx]-1] is None or 1 not in fill_dict[dimy][dimx][mosaic_bw[dimy-1][dimx]-1]) else 0, mosaic_bw[dimy+1][dimx] if mosaic_bw[dimy+1][dimx] != 0 and mosaic_bw[dimy+1][dimx] != proc_imgs2 and (fill_dict[dimy][dimx][mosaic_bw[dimy+1][dimx]-1] is None or 1 not in fill_dict[dimy][dimx][mosaic_bw[dimy+1][dimx]-1]) else 0]).most_common(2))[0]
                                if(proc_imgs2 != 0 and proc_imgs2 != proc_imgs):
                                    if(fill_dict[dimy][dimx][proc_imgs2-1] is None):
                                        fill_dict[dimy][dimx][proc_imgs2-1] = [1]
                                    else:
                                        fill_dict[dimy][dimx][proc_imgs2 -
                                                              1].append(1)
                ...
                if(temp_maskedges is not None and temp_maskedges[dimy2][dimx2] > temp_maskedges[dimy3][dimx3] and temp_maskedges[dimy2][dimx2] >= proc_imgs):
                    ...
                    if(temp_maskedges is not None and temp_maskedges[dimy3][dimx3] >= proc_imgs):
                        ...
                        if(nei1 == 2):
                            ...
                            threes = np.append(
                                threes, [[dimy2], [dimx2]], axis=1)
                            continue
                        if(nei2 == 2):
                            temp_mask[dimy3][dimx3] = 255
                            threes = np.append(
                                threes, [[dimy3], [dimx3]], axis=1)
                            continue
                        if(nei1 == nei2 and nei1 == 3):
                            ...
                            if(temp_maskall[dimy3][dimx3] == 255):
                                extras.append(
                                    [dimy3, dimx3, threes[0][x], threes[1][x], dimy1, dimx1])
                                block[dimy1][dimx1] = True
                                block[threes[0][x]][threes[1][x]] = True
                                continue
                        if(nei1 == 3):
...
def expand(mosaic_bw, current_upper_x, current_upper_y, left_upper_x, left_upper_y, real_img_shape, pixel_size):
    ...
    if (left_upper_x+(-1*real_img_shape[0]*pixel_size)) < (current_upper_x+(-1*mosaic_bw.shape[0]*pixel_size)):
        append_x_up = int(round(((current_upper_x+(-1*mosaic_bw.shape[0]*pixel_size))-(
            left_upper_x+(-1*real_img_shape[0]*pixel_size)))/pixel_size))
    ...
    if (left_upper_y+(real_img_shape[1]*pixel_size)) > (current_upper_y+(mosaic_bw.shape[1]*pixel_size)):
        append_y_up = int(round(((left_upper_y+(real_img_shape[1]*pixel_size))-(
            current_upper_y+(mosaic_bw.shape[1]*pixel_size)))/pixel_size))
    ...
    mosaic_bw = np.append(mosaic_bw, np.zeros(
        (abs(append_x_up), mosaic_bw.shape[1]), dtype=np.uint8), axis=0)
    mosaic_bw = np.append(np.zeros(
        (abs(append_x_down), mosaic_bw.shape[1]), dtype=np.uint8), mosaic_bw, axis=0)
    mosaic_bw = np.append(mosaic_bw, np.zeros(
        (mosaic_bw.shape[0], abs(append_y_up)), dtype=np.uint8), axis=1)
    mosaic_bw = np.append(np.zeros((mosaic_bw.shape[0], abs(
        append_y_down)), dtype=np.uint8), mosaic_bw, axis=1)
...
def draw(mosaic_bw, all_output, current_upper_x, current_upper_y, left_upper_x, left_upper_y, proc_imgs, real_img_shape, pixel_size, name):
    ...
    mask[mosaic_bw[int(x_disp):int(x_disp)+real_img_shape[0],
                   int(y_disp):int(y_disp)+real_img_shape[1]] > 0] = 0
...
def neighbors8_ex(temp_mask, curx, cury):
    a = int(temp_mask[curx+1][cury] > 0)+int(temp_mask[curx-1][cury] > 0)+int(temp_mask[curx][cury+1] > 0)+int(temp_mask[curx][cury-1] > 0) + \
        int(temp_mask[curx+1][cury+1] > 0)+int(temp_mask[curx-1][cury-1] > 0) + \
        int(temp_mask[curx-1][cury+1] > 0)+int(temp_mask[curx+1][cury-1] > 0)
    return a
...
def main():
    ...
    to_process = np.unique([int(os.path.basename(x).split("_")[0])
                            for x in glob.glob(args.img_path8bit+"/*.tif")]).tolist()
    print("Total files "+str(len(glob.glob(args.img_path8bit+"/*.tif"))))
    for vor_ind in range(min([int(os.path.basename(x).split("_")[0]) for x in glob.glob(args.img_path8bit+"/*.tif")]), max([int(os.path.basename(x).split("_")[0]) for x in glob.glob(args.img_path8bit+"/*.tif")])):
        filenames = [img for img in glob.glob(
            args.img_path8bit+"/*.tif") if int(os.path.basename(img).split("_")[0]) == vor_ind]
        vor_flag = 0
        for x in filenames:
            if not os.path.isfile(args.save_dir+"/mosaic_mask_"+args.dataset+"_"+os.path.basename(x)+".txt"):
                vor_flag = 1
                break
        filenames2 = [img for img in glob.glob(
            args.img_path8bit+"/*.tif") if int(os.path.basename(img).split("_")[0]) == vor_ind+1]
        for x in filenames2:
            if not os.path.isfile(args.save_dir+"/mosaic_mask_"+args.dataset+"_"+os.path.basename(x)+".txt"):
                vor_flag = 1
                break
            ...
            for x in filenames2:
                if os.path.isfile(args.save_dir+"/mosaic_mask_"+args.dataset+"_"+os.path.basename(x)+".txt"):
                    os.remove(args.save_dir+"/mosaic_mask_" +
                              args.dataset+"_"+os.path.basename(x)+".txt")
            break
    min_ind = min([int(os.path.basename(x).split("_")[0])
                   for x in glob.glob(args.img_path8bit+"/*.tif")])
    for vor_ind in range(min([int(os.path.basename(x).split("_")[0]) for x in glob.glob(args.img_path8bit+"/*.tif") if int(os.path.basename(x).split("_")[0]) in to_process]), max([int(os.path.basename(x).split("_")[0]) for x in glob.glob(args.img_path8bit+"/*.tif") if int(os.path.basename(x).split("_")[0]) in to_process])):
        filenamesn = [img for img in glob.glob(
            args.img_path8bit+"/*.tif") if int(os.path.basename(img).split("_")[0]) == vor_ind+1]
        filenamesn.sort()
        filenamesp = [img for img in glob.glob(
            args.img_path8bit+"/*.tif") if int(os.path.basename(img).split("_")[0]) == vor_ind]
        filenamesp.sort()
        if(min_ind != vor_ind):
        ...
        for im_fn_orig in filenames:
            ...
            if(os.path.isfile(args.save_dir+"/mosaic_mask_"+args.dataset+"_"+os.path.basename(im_fn_orig)+".txt")):
                f = open(args.save_dir+"/mosaic_mask_"+args.dataset +
                         "_"+os.path.basename(im_fn_orig)+".txt", "r")
                first_col = 0
            else:
                f = open(args.mosaiclines_path+"/mosaic_mask_" +
                         args.dataset+"_"+os.path.basename(im_fn_orig)+".txt", "r")
            contents = f.read()
            ...
            all_output = [[int((x[0]*(float(real_imgshape[0])/float(mosaic_x)))), int(
                (x[1]*(float(real_imgshape[1])/float(mosaic_y))))] for x in all_output]
            if os.path.isfile(im_fn_orig[:-4]+".tfw"):
                f = open(im_fn_orig[:-4]+".tfw", "r")
            else:
                f = open(im_fn_orig[:-4].replace("8bit", "")+".tfw", "r")
            ...
            if(min_ind != vor_ind and int(os.path.basename(im_fn_orig).split("_")[0]) == vor_ind):
                tfws[len(filenamesn)+((len(filenamesp)+1)-(proc_imgs-(len(filenamesn))))] = [
                    real_imgshape[0], real_imgshape[1], left_upper_y, left_upper_x, pixel_size]
            else:
                tfws[proc_imgs] = [real_imgshape[0], real_imgshape[1],
                                   left_upper_y, left_upper_x, pixel_size]
            if(mosaic_bw is None):
                mosaic_bw = np.zeros(
                    (real_imgshape[0], real_imgshape[1]), dtype=np.uint8)
                current_upper_x = left_upper_x
                current_upper_y = left_upper_y
            else:
                mosaic_bw, current_upper_x, current_upper_y = expand(
                    mosaic_bw, current_upper_x, current_upper_y, left_upper_x, left_upper_y, real_imgshape, pixel_size)
            if(min_ind != vor_ind and int(os.path.basename(im_fn_orig).split("_")[0]) == vor_ind):
        ...
        while(repeat == 1):
            ...
            ret, labels = cv2.connectedComponents(
                (mosaic_bw == 0).astype(np.uint8)*255)
            print("filling blank spaces: "+str(ret))
            if(drawerr == 2 and ret > 2):
                iterelem = np.array(np.unique(labels, return_counts=True)).T
                        ...
                        for filcont in range(4):
                            ...
                            for dircont in range(8):
                                ...
                                if(dircont == 7):
                                    dimy2 = dimy+1
                                    dimx2 = dimx-1
                                if(mosaic_bw[dimy2][dimx2] != 0 and (counts < 1 or (fill_dict[dimy][dimx] is None or fill_dict[dimy][dimx][mosaic_bw[dimy2][dimx2]-1] is None or counts not in fill_dict[dimy][dimx][mosaic_bw[dimy2][dimx2]-1])) and (counts < 1000 or check_bounds(mosaic_bw, dims, mosaic_bw[dimy2][dimx2], current_upper_x, current_upper_y, seen, tfws, 0, 0))):
            ...
            for im_fn_orig in filenames:
                ...
                if(proc_imgs not in houseimgs_bw):
                    ...
                    house_bw = ndimage.morphology.binary_dilation(
                        house_bw, structure=ndimage.generate_binary_structure(2, 2)).astype(np.uint8)*255
                    ...
                    y_disp = round((left_upper_y-current_upper_y)/pixel_size)
                    tfws[proc_imgs] = [real_imgshape[0], real_imgshape[1],
                                       left_upper_y, left_upper_x, pixel_size, x_disp, y_disp]
                else:
                    ...
                    x_disp = tfw[5]
                    y_disp = tfw[6]
                temp_mask = mosaic_bw[int(x_disp):int(
                    x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]].copy()
                ...
                if(drawerr == 2):
                    ...
                    edgesnew = ndimage.morphology.binary_dilation(
                        temp_edge, structure=ndimage.generate_binary_structure(2, 2)).astype(np.uint8)*255
                    edgesnew[mosaic_bw_edges[int(x_disp):int(
                        x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]] < proc_imgs] = 0
                    h, w = temp_mask.shape
                    seed = (1, 1)
                    mask = np.zeros((h+2, w+2), np.uint8)
                    floodflags = 8
                    ...
                    floodflags |= (255 << 8)
                    num, im, mask, rect = cv2.floodFill(
                        edgesnew, mask, seed, 255, 0, 0, floodflags)
                    mask = mask[1:-1, 1:-1]
                    mask = cv2.bitwise_not(mask)
                    del temp_mask
                    del edgesnew
                    ...
                    if(drawerr == 1):
                        ...
                        floodflags |= (255 << 8)
                        num, im, mask, rect = cv2.floodFill(
                            temp_mask, mask, seed, 255, 0, 0, floodflags)
                        mask = mask[1:-1, 1:-1]
                        mask = cv2.bitwise_not(mask)
                        temp_mask = mask
                ...
                if(im_fn_orig not in fixed3 and drawerr == 1):
                    ...
                    for lab in np.array(np.unique(labels, return_counts=False)).T:
                        if(lab != 0):
                            ...
                            dimx = dims[1][np.argmin(dims[0])]
                            reach = 1000
                            house_bwcrop = house_bw[max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(
                                dimx-reach, 0):min(dimx+reach, real_imgshape[1])].copy()
                            house_bwcrop[temp_mask[max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(
                                dimx-reach, 0):min(dimx+reach, real_imgshape[1])] == 0] = 0
                            house_bw[max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(dimx-reach, 0):min(dimx+reach, real_imgshape[1])][mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(
                                y_disp):int(y_disp)+real_imgshape[1]][max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(dimx-reach, 0):min(dimx+reach, real_imgshape[1])] == 0] = 0
                            ret, labelsfull = cv2.connectedComponents(house_bw[max(dimy-reach, 0):min(
                                dimy+reach, real_imgshape[0]), max(dimx-reach, 0):min(dimx+reach, real_imgshape[1])])
                            ret, labelscrop = cv2.connectedComponents(
                                house_bwcrop)
                            ...
                            if(im_fn_orig not in fixed3 and countall-countcrop < 1500 and countall-countcrop > 0):
                                ...
                                dimymin = min(dims2[1])
                                dimymax = max(dims2[1])
                                temp_wind = mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]][max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(
                                    dimx-reach, 0):min(dimx+reach, real_imgshape[1])][max(dimxmin-5, 0):min(dimxmax+5, real_imgshape[0]), max(dimymin-5, 0):min(dimymax+5, real_imgshape[1])].copy()
                                temp_wind_eyes = mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]][max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(
                                    dimx-reach, 0):min(dimx+reach, real_imgshape[1])][max(dimxmin-5, 0):min(dimxmax+5, real_imgshape[0]), max(dimymin-5, 0):min(dimymax+5, real_imgshape[1])].copy()
                                temp_wind_eyes[labelsfull[max(dimxmin-5, 0):min(dimxmax+5, real_imgshape[0]), max(
                                    dimymin-5, 0):min(dimymax+5, real_imgshape[1])] != curlab] = 0
                                ...
                                floodflags |= (255 << 8)
                                num, im, mask, rect = cv2.floodFill(
                                    temp_wind_eyes, mask, seed, 255, 0, 0, floodflags)
                                mask = mask[1:-1, 1:-1]
                                mask = cv2.bitwise_not(mask)
                                if(int(os.path.basename(im_fn_orig).split("_")[0]) == vor_ind and first_col == 0):
                                    mask_vor = mask.copy()
                                    mask_vor[mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]][max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(
                                        dimx-reach, 0):min(dimx+reach, real_imgshape[1])][max(dimxmin-5, 0):min(dimxmax+5, real_imgshape[0]), max(dimymin-5, 0):min(dimymax+5, real_imgshape[1])] == proc_imgs] = 0
                                    if(len(np.where((mask_vor > 0) & ((mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]][max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(dimx-reach, 0):min(dimx+reach, real_imgshape[1])][max(dimxmin-5, 0):min(dimxmax+5, real_imgshape[0]), max(dimymin-5, 0):min(dimymax+5, real_imgshape[1])] == 0) | (mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]][max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(dimx-reach, 0):min(dimx+reach, real_imgshape[1])][max(dimxmin-5, 0):min(dimxmax+5, real_imgshape[0]), max(dimymin-5, 0):min(dimymax+5, real_imgshape[1])] == nex_im) | (mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]][max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(dimx-reach, 0):min(dimx+reach, real_imgshape[1])][max(dimxmin-5, 0):min(dimxmax+5, real_imgshape[0]), max(dimymin-5, 0):min(dimymax+5, real_imgshape[1])] == prev_im)))[0]) > 0):
                                        continue
                                mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]][max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(dimx-reach, 0):min(
                                    dimx+reach, real_imgshape[1])][max(dimxmin-5, 0):min(dimxmax+5, real_imgshape[0]), max(dimymin-5, 0):min(dimymax+5, real_imgshape[1])][mask > 0] = proc_imgs
                                hflag = 1
                                temp_wind2 = mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]][max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(
                                    dimx-reach, 0):min(dimx+reach, real_imgshape[1])][max(dimxmin-5, 0):min(dimxmax+5, real_imgshape[0]), max(dimymin-5, 0):min(dimymax+5, real_imgshape[1])]
                                ...
                                if(im_fn_orig not in fixed3 and countall-countcrop > 0 and countcrop > 0 and countcrop < 1500):
                                    ...
                                    if(int(os.path.basename(im_fn_orig).split("_")[0]) == vor_ind and first_col == 0):
                                        if(len(np.where(mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]][max(dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(dimx-reach, 0):min(dimx+reach, real_imgshape[1])][dims] == 0)[0]) > 0):
                                            continue
                                    mosaic_bw[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]][max(
                                        dimy-reach, 0):min(dimy+reach, real_imgshape[0]), max(dimx-reach, 0):min(dimx+reach, real_imgshape[1])][dims] = 0
                                    hflag = 1
                                    if im_fn_orig in completed_images:
                ...
                temp_mask, removed_flag, temp_mask_ret, repeat_f = remove_singlets(temp_mask, mosaic_bw, filenames, current_upper_x, current_upper_y, tfws, fill_dict, x_disp, y_disp, completed_images, drawerr, ((
                    int(os.path.basename(im_fn_orig).split("_")[0]) == vor_ind) & (int(os.path.basename(im_fn_orig).split("_")[0]) != min_ind)), proc_imgs)
                if(repeat_f == 1):
                    repeat = 1
                ...
                if((len(completed_images)+1 == len(filenames)) and drawerr == 1 and repeat == 0):
                    ...
                    temp_mask, extras, completed_images, _ = remove_threes(temp_mask, temp_maskall, None, proc_imgs, mosaic_bw, filenames, completed_images, x_disp, y_disp, drawerr, fill_dict, (int(os.path.basename(
                        im_fn_orig).split("_")[0]) == vor_ind) & (int(os.path.basename(im_fn_orig).split("_")[0]) != min_ind), prev_im, nex_im, x_disp1, y_disp1)  # remove_threes(temp_mask,temp_maskall,None,proc_imgs)
                    temp_mask[temp_mask > 0] = 255
                    for x in extras:
                        temp_mask[x[0]][x[1]] = 255
                    edges_dict[proc_imgs] = np.where(temp_mask > 0)
                    temp_mask[mosaic_bw_edges[int(x_disp):int(
                        x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]] > proc_imgs] = 0
                    mosaic_bw_edges[int(x_disp):int(x_disp)+real_imgshape[0], int(
                        y_disp):int(y_disp)+real_imgshape[1]][temp_mask > 0] = proc_imgs
                    drawerr = 2
                    repeat = 1
                    del temp_mask
                    del temp_maskall
                    ...
                    break
                if(drawerr == 2):
                    temp_mask, extras, completed_images, repeat_f = remove_threes(temp_mask, temp_maskall, mosaic_bw_edges[int(x_disp):int(x_disp)+real_imgshape[0], int(y_disp):int(y_disp)+real_imgshape[1]], proc_imgs, mosaic_bw, filenames, completed_images, x_disp, y_disp, drawerr, fill_dict, (int(os.path.basename(
                        im_fn_orig).split("_")[0]) == vor_ind) & (int(os.path.basename(im_fn_orig).split("_")[0]) != min_ind), prev_im, nex_im, x_disp1, y_disp1)  # remove_threes(temp_mask,temp_maskall,mosaic_bw_edges[int(x_disp):int(x_disp)+real_imgshape[0],int(y_disp):int(y_disp)+real_imgshape[1]],proc_imgs)
                else:
                    temp_mask, extras, completed_images, repeat_f = remove_threes(temp_mask, temp_maskall, None, proc_imgs, mosaic_bw, filenames, completed_images, x_disp, y_disp, drawerr, fill_dict, (int(os.path.basename(
                        im_fn_orig).split("_")[0]) == vor_ind) & (int(os.path.basename(im_fn_orig).split("_")[0]) != min_ind), prev_im, nex_im, x_disp1, y_disp1)  # remove_threes(temp_mask,temp_maskall,None,proc_imgs)
                if(repeat_f == 1):
                    repeat = 1
                ...
                seen = {}
                last_index = 0
                all_output_save = np.append(
                    all_output_save, [[cury, curx, 0]], axis=0)
                seen[str([curx, cury])] = 1
                elapsed = 0
                cont_exit = 0
                now = datetime.now()
                while((not ([starty, startx] == [cury, curx] and len(all_output_save) > 14000)) and elapsed < 60):
                    cont_exit = cont_exit+1
                    if(cont_exit % 1000 == 0):
                        elapsed = (datetime.now() - now).total_seconds()
                    if(last_index < 1 and temp_mask[curx+1][cury] > 0 and (str([curx+1, cury]) not in seen or seen[str([curx+1, cury])] == -1 or ([starty, startx] == [cury, curx+1] and len(all_output_save) > 14000))):
                        curx = curx+1
                        cury = cury
                        seen[str([curx, cury])] = len(all_output_save)
                        all_output_save = np.append(
                            all_output_save, [[cury, curx, 1]], axis=0)
                        last_index = 0
                        continue
                    if(last_index < 2 and temp_mask[curx][cury+1] > 0 and (str([curx, cury+1]) not in seen or seen[str([curx, cury+1])] == -1 or ([starty, startx] == [cury+1, curx] and len(all_output_save) > 14000))):
                        curx = curx
                        cury = cury+1
                        seen[str([curx, cury])] = len(all_output_save)
                        last_index = 0
                        all_output_save = np.append(
                            all_output_save, [[cury, curx, 2]], axis=0)
                        continue
                    if(last_index < 3 and temp_mask[curx-1][cury] > 0 and (str([curx-1, cury]) not in seen or seen[str([curx-1, cury])] == -1 or ([starty, startx] == [cury, curx-1] and len(all_output_save) > 14000))):
                        curx = curx-1
                        cury = cury
                        seen[str([curx, cury])] = len(all_output_save)
                        last_index = 0
                        all_output_save = np.append(
                            all_output_save, [[cury, curx, 3]], axis=0)
                        continue
                    if(last_index < 4 and temp_mask[curx][cury-1] > 0 and (str([curx, cury-1]) not in seen or seen[str([curx, cury-1])] == -1 or ([starty, startx] == [cury-1, curx] and len(all_output_save) > 14000))):
                        curx = curx
                        cury = cury-1
                        seen[str([curx, cury])] = len(all_output_save)
                        last_index = 0
                        all_output_save = np.append(
                            all_output_save, [[cury, curx, 4]], axis=0)
                        continue
                    if(last_index < 5 and temp_mask[curx+1][cury+1] > 0 and (str([curx+1, cury+1]) not in seen or seen[str([curx+1, cury+1])] == -1 or ([starty, startx] == [cury+1, curx+1] and len(all_output_save) > 14000))):
                        curx = curx+1
                        cury = cury+1
                        seen[str([curx, cury])] = len(all_output_save)
                        last_index = 0
                        all_output_save = np.append(
                            all_output_save, [[cury, curx, 5]], axis=0)
                        continue
                    if(last_index < 6 and temp_mask[curx-1][cury-1] > 0 and (str([curx-1, cury-1]) not in seen or seen[str([curx-1, cury-1])] == -1 or ([starty, startx] == [cury-1, curx-1] and len(all_output_save) > 14000))):
                        curx = curx-1
                        cury = cury-1
                        seen[str([curx, cury])] = len(all_output_save)
                        last_index = 0
                        all_output_save = np.append(
                            all_output_save, [[cury, curx, 6]], axis=0)
                        continue
                    if(last_index < 7 and temp_mask[curx+1][cury-1] > 0 and (str([curx+1, cury-1]) not in seen or seen[str([curx+1, cury-1])] == -1 or ([starty, startx] == [cury-1, curx+1] and len(all_output_save) > 14000))):
                        curx = curx+1
                        cury = cury-1
                        seen[str([curx, cury])] = len(all_output_save)
                        last_index = 0
                        all_output_save = np.append(
                            all_output_save, [[cury, curx, 7]], axis=0)
                        continue
                    if(last_index < 8 and temp_mask[curx-1][cury+1] > 0 and (str([curx-1, cury+1]) not in seen or seen[str([curx-1, cury+1])] == -1 or ([starty, startx] == [cury+1, curx-1] and len(all_output_save) > 14000))):
                        curx = curx-1
                        cury = cury+1
                        seen[str([curx, cury])] = len(all_output_save)
                        last_index = 0
                        all_output_save = np.append(
                            all_output_save, [[cury, curx, 8]], axis=0)
                        continue
                ...
                for x in extras:
                    pos = np.where(
                        np.all(all_output_save == [x[3], x[2]], axis=1))[0]
                    if(x[4] is None):
                        pos2 = -1
                    else:
                        pos2 = np.where(
                            np.all(all_output_save == [x[5], x[4]], axis=1))[0]
                    ...
                    all_output_save = np.insert(
                        all_output_save, max(pos, pos2), [x[1], x[0]], axis=0)
                    temp_mask[x[0]][x[1]] = 255
                totnodes = len(all_output_save)
                all_output_save_real = [[[(left_upper_y+(all_output_save[x][0]*pixel_size)), (left_upper_x-(all_output_save[x][1]*pixel_size))], [(left_upper_y+(
                    all_output_save[(x+1) % totnodes][0]*pixel_size)), (left_upper_x-(all_output_save[(x+1) % totnodes][1]*pixel_size))]] for x in range(len(all_output_save))]
                if(repeat == 0 and drawerr == 2):
                    f = open(args.save_dir+"/mosaic_mask_tmp_"+args.dataset +
                    ...
                    f.close()
                    w = shapefile.Writer(
                        args.save_dir+os.path.basename(im_fn_orig), shapeType=3)
        ...
        houseimgs_bw.clear()
        del houseimgs_bw
        print("Finished generating mosaic lines for "+str(filenamesp))
